...
class AuditController extends Controller
{
    public function titleNovelIndex(Request $request)
    {
        ...
        })->where('b.id', 2)->whereIn('user_id', $userGroup)->whereIn('status',[0,2])->select(['a.*','b.name','c.name as user_name'])->orderBy('status')->paginate(15);
    ...
    public function updateTitleSave(Request $request)
    {
        ...
        if (is_array($id)){
            $ids = $id;
            foreach ($ids as $id){
                DB::connection('admin')->table('wechat_title_info')->where('id', $id)->update(['status' => $status, 'msg' => $reason, 'audit_time' => date('Y-m-d H:i:s')]);
            }
        }else {
            DB::connection('admin')->table('wechat_title_info')->where('id', $id)->update(['status' => $status, 'msg' => $reason, 'audit_time' => date('Y-m-d H:i:s')]);
        }
